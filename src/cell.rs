...
pub fn unlink (cell1: CellLink, cell2: CellLink) {
  ...
  match cell1_borrowed.links.iter().position(|v| *v.borrow() == *cell2.borrow()) {
    Some(index) => { cell1_borrowed.links.remove(index); },
    None => {}
  };
  
  let mut cell2_borrowed = cell2.borrow_mut();
  match cell2_borrowed.links.iter().position(|v| *v.borrow() == *cell1.borrow()) {
    Some(index) => { cell2_borrowed.links.remove(index); },
    None => {}
  };
}